...
class FUNC {
	...
	static function str_clear($strval, $addrep = array()){
		$ret = '';
		$rep = array("\r", "\n", "\t");
	...
	static function is_alpha($strval, $numeric = true, $addstr = ''){
	...
	static function is_hostname($strhost){
		$ret = false;
	...
	static function getmxrr_win($hostname, &$mxhosts){
		$mxhosts = array();
			...
			if(FUNC::is_hostname($hostname)){
				...
				if($retstr && count($retarr) > 0){
					foreach($retarr as $line){
						if(eregi('/.*mail exchanger = (.*)/', $line, $matches)) $mxhosts[] = $matches[1];
					}
				}
	...
	static function is_connection($connection){
		$ret = false;
	...
	static function is_mail($addr, $vermx = false){
		$ret = false;
		if(is_string($addr) && $addr != ""){
			$regs = '^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,6})$';
			if(preg_match("/".$regs."/i", $addr)){
				if(is_bool($vermx)){
					if($vermx){
						$exp = explode('@', $addr);
	...
	static function split_reverse($body, $multipart, $boundary){
		$ret = array();
	...
	static function split_content($str){
		$ret = false;
	...
	static function decode_content($str, $decode = "base64"){
